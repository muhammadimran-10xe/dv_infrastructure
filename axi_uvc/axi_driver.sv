class axi_driver extends uvm_driver #(axi_transaction);
    ...
    task reset_all();
      vif.awvalid_i <= 0; vif.awaddr_i  <= '0;
      vif.wvalid_i  <= 0; vif.wdata_i   <= '0; vif.wstrb_i <= 4'hF;
      vif.bready_i  <= 0;
      vif.arvalid_i <= 0; vif.araddr_i  <= '0;
      vif.rready_i  <= 0;
    endtask
    ...
    task drive_write(axi_transaction trans);
        ...
        @(posedge vif.clk_i);
        wait(vif.awready_o && vif.wready_o);
        vif.awvalid_i <= 0;
        vif.wvalid_i  <= 0;
        vif.bready_i <= 1;
    ...
    task drive_read(axi_transaction trans);
        @(posedge vif.clk_i);
        vif.arvalid_i <= 1; vif.araddr_i <= trans.addr;
