class axi_simple_seq extends uvm_sequence #(axi_transaction);
    ...
    task body();
        ...
        `uvm_info(get_type_name(),"=== SPI MOSI Test: first config regs then sending data ===",UVM_LOW)
        ...
        axi_write(`SPI_CR, 32'h0000_0106);   // CR at 0x60
 
        `uvm_info(get_type_name(), "Load TX FIFO (DTR=0xA)", UVM_LOW)
        axi_write(`SPI_DTR, 32'h0000_000A);   // DTR at 0x68
 
        `uvm_info(get_type_name(),"START transfer (CR=0x086, clears TRANS_INHIBIT)", UVM_LOW)
        axi_write(`SPI_CR, 32'h0000_0006);   // CR at 0x60

        axi_read(`SPI_SR);                  // 0x64

        axi_read(`SPI_DRR);                 // 0x6c

        axi_write(`SPI_CR, 32'h0000_0106);   // CR back to INHIBIT=1
